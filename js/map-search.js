        ...
        window.MapSearchEngine = {
            ...
            async searchByCategory(category) {
                ...
                const osmTags = {
                    ...
                    attraction: '[tourism~"museum|attraction|monument"]',
                    all:        '[amenity~"hospital|pharmacy|restaurant|police"]'
                };
                
                const tag = osmTags[category] || osmTags.all;
                ...
                try {
                    this.showNotification('Ищем места...', 'info');
                    ...
                    const pois = data.elements.map(el => ({
                        ...
                        category: category,
                        name: { 
                            ru: el.tags?.['name:ru'] || el.tags?.name || 'Без названия',
                            en: el.tags?.['name:en'] || el.tags?.name || 'No name'
                        },
                        address: [
                            el.tags?.['addr:street'],
                    ...
                    if (!pois.length) {
                        this.showNotification('Ничего не найдено поблизости', 'info');
                        return;
                    }
                    
                    this.displayPOIs(pois);
                    this.showNotification(`Найдено: ${pois.length}`, 'success');
        ...
        window.MapPanel = {
            ...
            open() {
                ...
                setTimeout(() => {
                    ...
                    setTimeout(() => {
                        if (this.panelMap) this.panelMap.invalidateSize();
                    }, 300);
